...
#%%
subset = ref_df.loc[(ref_df["ice"]=="AMN") | (ref_df["ice"]=="EQB")].reset_index()
control_subset = ref_df.loc[ref_df["ice"]=="PRD"].reset_index()
datasets = [tavgs[i] for i in subset["index"]]
control_datasets = [tavgs[i] for i in list(control_subset["index"])*2] 
#%%
def surface_ver4(var, cmap, vmin, vmax, lev = 0, proj = ccrs.SouthPolarStereo(), nbound = -45, contour = None, levels = None,  title=""): 
    ...
    control_datasets = [get_2d_slice(tavgs[i][var], lev) for i in (list(control_subset["index"])*2)]  

    fig, axs = plt.subplots(1, 4, figsize=figsize, subplot_kw={'projection': proj})
    ...
    for i, ax in enumerate(axs.flat):
        ...
        ax.set_title(subset["orb"][i] + ", " + subset["ice"][i] + " - PRD", fontsize=10, pad=2)
...
#surface_ver4("O_temp", "coolwarm", -1, 1,  nbound = -45, contour = None, levels = None,  title="SST")
surface_ver4("O_temp", "coolwarm", -1, 1, proj = ccrs.PlateCarree(), nbound = 90, contour = None, levels = None,  title="SST")

#%%
def transect_ver4(var, basin, cmap, vmin,vmax,contour = None, extent="Global",  title="", datasets = datasets, control_datasets=control_datasets):
    if extent == "SO":
        xlim=(-90, -50)
        xextent = 24
        figsize = (10,6)
    ...
    fig, axs = plt.subplots(1, 4, figsize=figsize, sharey = True, sharex =True)
...
transect_ver4("O_sal", "Global", "PiYG", -0.1,0.1,contour = "density", extent ="Global",  title="")

# %%
transect_ver4("O_temp", "Global", "coolwarm", -1,1,contour = "density", extent ="Global",  title="")
surface_ver4("O_temp", "coolwarm", -1, 1,  nbound = -45, contour = None, levels = None,  title="SST")
surface_ver4("O_temp", "coolwarm", -1, 1, proj = ccrs.PlateCarree(), nbound = 90, contour = None, levels = None,  title="SST")
surface_ver4("O_temp", "coolwarm", -1, 1, proj = ccrs.NorthPolarStereo())

# %%
transect_ver4("O_sal", "Global", "PiYG", -0.1,0.1,contour = "density", extent ="Global",  title="")
surface_ver4("O_sal", "PiYG", -0.5,0.5,  nbound = -45, contour = None, levels = None,  title="")
surface_ver4("O_sal", "PiYG", -0.5,0.5, proj = ccrs.PlateCarree(), nbound = 90, contour = None, levels = None,  title="")

# %%
transect_ver4("O_temp", "Global", "coolwarm", -1,1,contour = "density", extent ="Global",  title="")
transect_ver4("O_temp", "Atlantic", "coolwarm", -1,1,contour = "density", extent ="SO",  title="Atlantic")
transect_ver4("O_temp", "Pacific", "coolwarm", -1,1,contour = "density", extent ="SO",  title="Pacific")
transect_ver4("O_temp", "Indian", "coolwarm", -1,1,contour = "density", extent ="SO",  title="Indian")
# %%
transect_ver4("O_sal", "Global", "PiYG", -0.1,0.1, contour ="density", extent ="Global",  title="")
transect_ver4("O_sal", "Atlantic", "PiYG", -0.1,0.1, contour ="density", extent ="SO",  title="Atlantic")
transect_ver4("O_sal", "Pacific", "PiYG", -0.1,0.1, contour ="density", extent ="SO",  title="Pacific")
transect_ver4("O_sal", "Indian", "PiYG", -0.1,0.1, contour ="density", extent ="SO",  title="Indian")
# %%
surface_ver4("A_sat", "coolwarm", -1, 1,proj = ccrs.PlateCarree(), title="")
surface_ver4("A_sat", "coolwarm", -1, 1,proj = ccrs.SouthPolarStereo(), title="")

# %%
surface_ver4("A_awindY", "managua", -0.1, 0.1,proj = ccrs.SouthPolarStereo(), title="")
surface_ver4("A_awindX", "managua", -0.1, 0.1,proj = ccrs.SouthPolarStereo(), title="")
surface_ver4("O_tauX", "managua", -0.005, 0.005,proj = ccrs.SouthPolarStereo(), title="")
...
def plot_ovt(basin, title, save=False, path = None, datasets = datasets, control_datasets=control_datasets):
    ...
    fig, axs = plt.subplots(5,1, figsize=(8,10), sharex =True, sharey =True, layout='constrained')
    ...
    for i, ax in enumerate(axs.flat):
        ...
        if i==0:
            ...
            ax.clabel(c1, c1.levels, inline=True, fontsize=10)
            ax.set_title(subset["orb"][i-1] + ", " + subset["ice"][i-1] + " - PRD", fontsize=10, pad=2)
            ax.set_ylabel("Depth")
...
data = tavgs[5]["F_solins"].values - tavgs[1]["F_solins"].values
        
ax.set_extent([-180, 180,south_bound, north_bound], ccrs.PlateCarree())
ax.coastlines()
ax.gridlines(linestyle='--', draw_labels=False, color="gray")
cm = ax.pcolormesh(lons, lats, data[0,:,:], transform=ccrs.PlateCarree(),
                vmin=-0.1, vmax=0.1, 
                cmap= "Spectral_r", shading="auto")#, norm=colors.SymLogNorm(linthresh=0.000001, linscale=1, vmin=vmin, vmax=vmax))
cb = fig.colorbar(cm, ax=ax, orientation='horizontal', fraction=0.02, pad=0.07)
# %%
